...
async function market(){const students=await listAll("students");const opportunities=await listOpportunities();const skillMap=new Map(),roleMap=new Map(),skillDemand=new Map();for(const s of students){for(const skill of s.skills||[])skillMap.set(skill,(skillMap.get(skill)||0)+1);for(const role of(s.roles||[]).filter(Boolean))roleMap.set(role,(roleMap.get(role)||0)+1);}for(const o of opportunities)for(const skill of o.skills||[])skillDemand.set(skill,(skillDemand.get(skill)||0)+1);return{students:students.length,opportunities:opportunities.length,skills:[...new Set([...skillMap.keys(),...skillDemand.keys()])].map(skill=>({skill,students:skillMap.get(skill)||0,openings:skillDemand.get(skill)||0})).sort((a,b)=>b.students-a.students),roles:[...roleMap].map(([role,students])=>({role,students})).sort((a,b)=>b.students-a.students),years:[...new Set(students.map(s=>s.year).filter(Boolean))].sort()};}
...
if(url.pathname==="/api/opportunities"&&req.method==="GET"){let rows=await listOpportunities();const q=(url.searchParams.get("q")||"").toLowerCase();const role=(url.searchParams.get("role")||"").toLowerCase();const mode=(url.searchParams.get("mode")||"").toLowerCase();const year=url.searchParams.get("year");const skill=(url.searchParams.get("skill")||"").toLowerCase();const company=(url.searchParams.get("company")||"").toLowerCase();const location=(url.searchParams.get("location")||"").toLowerCase();const experience=Number(url.searchParams.get("experience")||0);const minStipend=Number(url.searchParams.get("minStipend")||0);const industry=(url.searchParams.get("industry")||"").toLowerCase();const sort=url.searchParams.get("sort")||"match";
if(q)rows=rows.filter(o=>`${o.company} ${o.title} ${o.description} ${(o.skills||[]).join(" ")} ${(o.interests||[]).join(" ")}`.toLowerCase().includes(q));
if(role)rows=rows.filter(o=>`${o.title} ${(o.roles||[]).join(" ")}`.toLowerCase().includes(role));
if(mode)rows=rows.filter(o=>String(o.mode||"").toLowerCase()===mode);
if(skill)rows=rows.filter(o=>(o.skills||[]).some(s=>String(s).toLowerCase()===skill));
if(company)rows=rows.filter(o=>String(o.company||"").toLowerCase().includes(company));
if(location)rows=rows.filter(o=>String(o.location||"").toLowerCase().includes(location));
if(year)rows=rows.filter(o=>(o.targetYears||[]).map(String).includes(String(year)));
if(experience)rows=rows.filter(o=>Number(o.maxExperienceMonths||9999)>=experience || Number(o.minExperienceMonths||0)<=experience);
if(minStipend)rows=rows.filter(o=>{const n=Number(String(o.stipend||"").replace(/[^0-9.]/g,""));return Number.isFinite(n)&&n>=minStipend;});
if(industry)rows=rows.filter(o=>String(o.industry||"").toLowerCase().includes(industry));
if(sort==="stipend")rows.sort((a,b)=>Number(String(b.stipend||"").replace(/[^0-9.]/g,""))-Number(String(a.stipend||"").replace(/[^0-9.]/g,"")));
else if(sort==="newest")rows.sort((a,b)=>String(b.createdAt||"").localeCompare(String(a.createdAt||"")));
else if(sort==="title")rows.sort((a,b)=>String(a.title).localeCompare(String(b.title)));
return json(res,200,rows);}
if(url.pathname==="/api/opportunities"&&req.method==="POST"){if(user.role!=="organization")return json(res,403,{error:"Organization account required"});const d=await readBody(req);if(!d.company||!d.title||!d.description)return json(res,400,{error:"Company, role and description are required"});const parsed=parseJobText(`${d.title} ${d.description} ${(d.skills||[]).join(" ")}`);return json(res,201,await createOpportunity({...d,skills:[...new Set(cleanList(d.skills).concat(parsed.skills))],interests:cleanList(d.interests),roles:parsed.roles,organizationUid:user.uid,targetYears:d.targetYears||[],industry:d.industry||"Technology",createdAt:new Date().toISOString()}));}
...
if(url.pathname.startsWith("/api/matches/")&&req.method==="GET"){const id=Number(url.pathname.split("/").pop());const s=await getStudent(id,user.uid);if(!s)return json(res,404,{error:"Student not found"});return json(res,200,{student:s,matches:rankMatches(s,await listOpportunities())});}
...
if(url.pathname==="/api/applications"&&req.method==="POST"){if(user.role!=="student")return json(res,403,{error:"Student account required"});const d=await readBody(req);const o=await getOpportunity(Number(d.opportunityId));if(!o)return json(res,404,{error:"Opportunity not found"});const c=await collection("applications");const existing=await c.findOne({uid:user.uid,opportunityId:o.id});if(existing)return json(res,200,noId(existing));const a={id:`app-${Date.now()}-${Math.random().toString(36).slice(2,7)}`,uid:user.uid,opportunityId:o.id,company:o.company,title:o.title,status:"Applied",note:d.note||"",createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()};await c.insertOne(a);return json(res,201,a);}
if(url.pathname.startsWith("/api/applications/")&&req.method==="PUT"){if(user.role!=="student")return json(res,403,{error:"Student account required"});const id=url.pathname.split("/").pop();const d=await readBody(req);const c=await collection("applications");const item=await c.findOne({uid:user.uid,id});if(!item)return json(res,404,{error:"Application not found"});const allowed=["Saved","Applied","Shortlisted","Interview","Offer","Rejected","Withdrawn"];if(d.status&&!allowed.includes(d.status))return json(res,400,{error:"Invalid application status"});const next={...item,status:d.status||item.status,note:d.note!==undefined?String(d.note):item.note,updatedAt:new Date().toISOString()};await c.findOneAndUpdate({uid:user.uid,id},{$set:{status:next.status,note:next.note,updatedAt:next.updatedAt}},{returnDocument:"after"});return json(res,200,next);}
if(url.pathname.startsWith("/api/applications/")&&req.method==="DELETE"){if(user.role!=="student")return json(res,403,{error:"Student account required"});const id=url.pathname.split("/").pop();const c=await collection("applications");await c.deleteOne({uid:user.uid,id});return json(res,200,{ok:true});}
