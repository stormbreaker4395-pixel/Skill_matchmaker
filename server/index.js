...
async function requireAuth(req, res) {
  ...
  try {
    ...
    json(res, 401, { error: "Invalid or expired authentication token" });
    return null;
  }
}
  ...
  try {
    ...
    if (url.pathname.startsWith("/api/")) {
      ...
      if (req.method === "GET" && url.pathname === "/api/students") {
        const students = await listStudents(uid);

        if (students.length === 0) {
          const student = await ensureStudentForUser(uid, email);
      ...
      if (req.method === "GET" && url.pathname.startsWith("/api/students/")) {
        ...
        const student = await getStudent(id, uid);
      ...
      if (req.method === "POST" && url.pathname === "/api/students") {
        ...
        return json(res, 201, await createStudent(data, uid));
      }
      ...
      if (req.method === "PUT" && url.pathname.startsWith("/api/students/")) {
        ...
        const updated = await updateStudent(id, data, uid);
